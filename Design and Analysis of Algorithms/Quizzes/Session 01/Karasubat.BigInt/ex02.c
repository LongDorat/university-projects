...
big_int divine_and_conquer_multiply(big_int x, big_int y)
{
    ...
    big_int ac = divine_and_conquer_multiply(a, c);
    big_int ad = divine_and_conquer_multiply(a, d);
    big_int bc = divine_and_conquer_multiply(b, c);
    big_int bd = divine_and_conquer_multiply(b, d);
    ...
    {
        result.digits[i + half_length] += ad.digits[i];
    }
    for (int i = 0; i < bc.size; ++i)
    {
        result.digits[i + half_length] += bc.digits[i];
    }
