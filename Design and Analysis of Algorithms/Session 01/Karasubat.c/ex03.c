...
long karasubat_multiply(long x, long y)
{
    if (x == 0 || y == 0)
    {
        return 0;
    ...
    long ac = karasubat_multiply(a, c);
    long bd = karasubat_multiply(b, d);
    long abcd = karasubat_multiply(a + b, c + d);

    return ac * pow10_int(2 * half_length) + ((abcd - ac - bd) * power) + bd;
}
